# --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
### In this file, all models (excluding the classical ratio model) for the DV cost_of_employees from the Dutch Structural Business are fitted.
...
str(dat)
dat_no_outliers = dat[-c(38, 263,  271, 103, 24), ] # Main outliers exclusion 
dat_no_outliers <- dat_no_outliers[complete.cases(dat_no_outliers),]
colnames(dat_no_outliers)[colnames(dat_no_outliers)=="Costs_purchases"] <- "y" # Defining outcome variable
...
ggsave('Business_costpurchases.png', plot=last_plot(), width=10, height=5.5)
...
print(xtable(accuracy_depr, type="latex", file="business_costppurchases.tex")) # boosted models' accuracy
print(xtable(accuracy_rlm_final, type="latex", file="business_costppurchases.tex")) # non-boosted models' accuracy